...
public class WebController {
    ...
    private User getLoggedInUser(Authentication authentication) {
        ...
        try {
            ...
            System.err.println("Error finding logged in user: " + username + " - " + e.getMessage());
            return null;
        }
    }
    ...
    public String handleLogMeal(Authentication authentication,
                                @RequestParam Long foodId,
                                @RequestParam String mealType,
                                @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        User user = getLoggedInUser(authentication);
        if (user != null) {
            foodLogService.logMeal(user.getUserId(), foodId, mealType, date);
        }
        return "redirect:/dashboard";
    }

    @PostMapping("/plan-meal")
    public String handlePlanMeal(Authentication authentication,
                                 @RequestParam Long foodId,
                                 @RequestParam String mealType,
                                 @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        User user = getLoggedInUser(authentication);
        if (user != null) {
            mealPlanningService.createPlan(user.getUserId(), foodId, date, mealType);
        }
        return "redirect:/dashboard";
    }
    ...
    public String handleEatPlannedMeal(@PathVariable Long logId, Authentication authentication) {
        User user = getLoggedInUser(authentication);
        if (user != null) {
            foodLogService.updateLogStatusToEaten(logId);
        }
        return "redirect:/dashboard";
    }
    ...
    @PostMapping("/report")
    public String handleGenerateReport(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
    ...
    public String handleUpdateProfile(Authentication authentication,
